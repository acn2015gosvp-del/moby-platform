    ...
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          // 실제 알림 메시지만 처리 (CRITICAL, WARNING, NOTICE, RESOLVED)
          if (!['CRITICAL', 'WARNING', 'NOTICE', 'RESOLVED'].includes(rawData.type)) {
            console.log('[useWebSocket] 알 수 없는 메시지 타입 (무시):', rawData.type)
            return
          }
          
          // 백엔드가 보내는 형식: { type, message, color, device_id, timestamp }
          // 프론트엔드 Alert 형식으로 변환
          const alertData: Alert = {
            id: rawData.id || `ws-${Date.now()}-${Math.random()}`,
            level: rawData.type?.toLowerCase() || rawData.level || 'info',
            message: rawData.message || '알림 없음',
            sensor_id: rawData.device_id || rawData.sensor_id || 'unknown',
