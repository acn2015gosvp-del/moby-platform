...
### 1. 추가 테스트 커버리지 향상 (우선순위: 중간)

#### 현재 커버리지가 낮은 모듈
- `grafana_client.py`: 17% → 목표 50% 이상
- `routes_grafana.py`: 42% → 목표 70% 이상

#### 작업 내용
- [ ] Grafana 클라이언트 단위 테스트 추가
- [ ] Grafana 라우터 통합 테스트 추가
- [ ] 에러 케이스 테스트 추가
- [ ] Mock을 사용한 외부 의존성 테스트
...
- ⚠️ `grafana_client.py`: 17% (개선 필요)
- ⚠️ `routes_grafana.py`: 42% (개선 필요)
