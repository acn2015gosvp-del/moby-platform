...
class TestGrafanaRoutes:
    ...
    def test_create_datasource_with_empty_name(self, client, mock_grafana_client):
        """빈 이름으로 데이터 소스 생성 시도"""
    ...
    def test_create_datasource_with_invalid_type(self, client, mock_grafana_client):
        """잘못된 타입으로 데이터 소스 생성"""
        mock_grafana_client.create_datasource.return_value = {
    ...
    def test_get_datasource_with_special_characters(self, client, mock_grafana_client):
        ...
        with patch('backend.api.routes_grafana.get_grafana_client', return_value=mock_grafana_client):
            response = client.get("/grafana/datasources/Test-DS_123")
            
            assert response.status_code == 200
            data = response.json()
            assert data["data"]["name"] == "Test-DS_123"
    
    def test_get_datasource_with_unicode_name(self, client, mock_grafana_client):
        ...
        with patch('backend.api.routes_grafana.get_grafana_client', return_value=mock_grafana_client):
            ...
            response = client.get(f"/grafana/datasources/{encoded_name}")
            
            assert response.status_code == 200
            data = response.json()
            assert data["data"]["name"] == "테스트 데이터소스"
    
    def test_create_dashboard_with_empty_title(self, client, mock_grafana_client):
        """빈 제목으로 대시보드 생성 시도"""
        # 빈 문자열은 Pydantic에서 허용되므로 실제 Grafana API에서 처리됨
        mock_grafana_client.create_sensor_dashboard.return_value = {
    ...
    def test_create_dashboard_with_very_long_title(self, client, mock_grafana_client):
        """매우 긴 제목으로 대시보드 생성"""
        long_title = "A" * 1000
        mock_grafana_client.create_sensor_dashboard.return_value = {
    ...
    def test_create_datasource_missing_required_fields(self, client, mock_grafana_client):
    ...
    def test_get_datasource_with_nonexistent_name(self, client, mock_grafana_client):
        """존재하지 않는 데이터 소스 조회"""
        mock_grafana_client.get_datasource_by_name.return_value = None
        
        with patch('backend.api.routes_grafana.get_grafana_client', return_value=mock_grafana_client):
            response = client.get("/grafana/datasources/NonExistentDS")
            
            assert response.status_code == 404
            data = response.json()
            assert "데이터 소스를 찾을 수 없습니다" in data["detail"]
    
    def test_create_datasource_with_all_optional_fields(self, client, mock_grafana_client):
    ...
    def test_create_dashboard_with_custom_datasource_name(self, client, mock_grafana_client):
    ...
    def test_get_datasource_http_exception_propagation(self, client, mock_grafana_client):
        """데이터 소스 조회 중 HTTP 예외 전파"""
        import httpx
        mock_grafana_client.get_datasource_by_name.side_effect = httpx.HTTPError("Connection failed")
        
        with patch('backend.api.routes_grafana.get_grafana_client', return_value=mock_grafana_client):
            response = client.get("/grafana/datasources/Test")
            
            assert response.status_code == 500
            data = response.json()
            assert "데이터 소스 조회 실패" in data["detail"]
    
    def test_create_datasource_http_exception_propagation(self, client, mock_grafana_client):
    ...
    def test_create_dashboard_http_exception_propagation(self, client, mock_grafana_client):
